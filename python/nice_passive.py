...
if __name__ == "__main__":
    # اگر آرگومان فایل بود
    if len(sys.argv) > 1 and is_file(sys.argv[1]):
        with open(sys.argv[1], 'r') as file:
            lines = file.readlines()
    # اگر از stdin خونده می‌شه (مثل cat file.txt | python3 ...)
    elif not sys.stdin.isatty():
        lines = sys.stdin.read().strip().splitlines()
    # اگر دامنه به‌صورت آرگومان داده شده
    elif len(sys.argv) > 1:
        lines = [sys.argv[1]]
    else:
        print(f"Usage:")
        print(f"  echo domain.com | nice_passive")
        print(f"  cat domains.txt | nice_passive")
        print(f"  nice_passive domains.txt")
        sys.exit()

    # اجرای passive برای هر خط
    for line in lines:
        domain = get_hostname(line.strip())
        if domain:
